...
#### Code comments
Are comments in code useful? Some say they should be avoided(避免) as much as possible, and hopefully made unnecessary. Do you agree?
...
C++ supports multiple inheritance, and Java allows a class to implement multiple interfaces. What impact(影响) does using these facilities(特性) have on orthogonality(正交性)? Is there a difference in impact between using multiple inheritance and multiple interfaces? Is there a difference between using delegation and using inheritance? [This question is from The Pragmatic Programmer, by Andrew Hunt and David Thomas]

#### Domain Logic in Stored Procedures
What are the pros and cons(利弊) of holding domain logic in Stored Procedures(存储过程)?
...
#### 3 worst defects
Tell me the 3 worst defects(缺点) of your preferred language

#### Functional Programming
Why is there a rising(增长) interest on Functional Programming?

#### Closures
What is a closure(闭包), and what is useful for? What's in common between closures and classes?

#### Generics
What are generics(泛型) useful for?

#### High-Order Functions
What are higher-order functions? What are they useful for? Write one, in your preferred language.

#### Loops and Recursion(递归)
Write a loop, then transform it into a recursive function, using only immutable(不可变) structures (i.e. avoid using variables). Discuss.
...
#### Anonymous Functions
Show me an example where an anonymous(匿名) function can be useful.

#### Static and Dynamic typing
There are a lot of different type systems. Let's talk about static and dynamic type systems, and about strong and weak ones. You surely have an opinion and a preference about this topic. Would you like to share them, and discuss why and when would you promote(促进) one particular(特定) type system for developing an enterprise software?

#### Namespaces
What are namespaces useful for? Invent an alternative.

#### Language Interoperability
Talk about interoperability(互操作性) between Java and C# (in alternative, choose 2 other arbitrary languages)
...
Write two functions, one referentially transparent(引用透明) and the other one referentially opaque(引用不透明). Discuss.

#### Stack and Heap
What is a stack and what is a heap? What's a stack overflow(溢出)?

#### Pattern Matching
Some languages, especially(特别是) the ones that promote(推崇) a functional approach(方式), allow a technique called pattern(模式) matching. Do you know it? How is pattern matching different from switch clauses?
